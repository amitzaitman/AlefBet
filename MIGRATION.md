...
### תוצאת הביצוע (ענפים מוכנים לסקירה; טרם מיזוג)

| שלב | PR / ענף | מצב |
|---|---|---|
| 1 | #4 | lifecycle, שמע וערבוב משותפים |
| 2 | #5 | פיצול כניסות עם תאימות |
| 3 | #6 | עורך עצל, תוכן עצמאי ו-precache לפי תלויות הבנייה |
| 4a | #7 | runGame + הברות |
| 4b | #8 | אותיות + תבנית עובדת |
| 4c | #9 | גרירה + פירוק משאבי סיבוב |
| 4d | codex/migration-4d-cleanup-docs | דיבור נשאר עם מדיניות שלושה ניסיונות; אולפן מחוץ למנהל |
| 5–6 | נדחה במכוון | אין צורך כרגע בשכתוב האתר או בהסרת החוזה הישן |

למזג לפי הסדר, ולעדכן את בסיס ה-PR הבא ל-main אחרי כל מיזוג. כל PR
מכיל שלב אחד מעל קודמו. תיבות הסימון למעלה מתעדות מיזוג, לא רק כתיבת קוד.
אימות אוטומטי: 396 בדיקות יחידה/אינטגרציה, 18 בדיקות Playwright ב-Chromium 149,
ובנייה חוזרת. בדיקות הדפדפן כוללות אופליין, טעינת העורך לפי בקשה,
תוכן שמור, סיום והפעלה מחדש ללא הצטברות מאזיני שמע. נבדק גם רוחב 390px.
הבדיקה בטלפון אמיתי (ללא רשת וכולל שמע) עדיין ידנית; לא בוצעה כאן.

הכרעות יישום: `createRoundManager` נשאר מימוש פנימי של runGame וייצוא
לתאימות. רק runtime נטען במשחקים; editor נטען בלחיצה ולא באתחול. מודל
התוכן הועבר ל-core עם re-exports כדי לשמור נתוני localStorage קיימים.
הבנייה הנוספת היא בניית ספרייה בלבד; האתר נשאר סטטי ו-start.bat נשאר תקין.
