...
## מצב מעודכן — 2026-09-07

שלבים 0–4 מוזגו ל-main ב-PRs #4–#10; תיקון משחק האותיות מוזג ב-#11.
הסעיפים ההיסטוריים בהמשך מתארים את המצב לפני המיגרציה, ואינם רשימת עבודה פתוחה.

- [x] 0 מקור אמת אחד לקטלוג המשחקים
- [x] 1 עזרי שמע, ערבוב וניהול מחזור חיים משותפים
- [x] 2 כניסות runtime/editor נפרדות עם תאימות לכניסה הישנה
- [x] 3 טעינת העורך בלחיצה בלבד
- [x] 4a runGame + syllable-read
- [x] 4b letter-match-animals + תבנית
- [x] 4c nikud-match + פירוק משאבי סיבוב
- [x] 4d nikud-speak שומר מדיניות מקומית; sound-studio מחוץ למנהל הסיבובים
- [ ] 5 Vite לאתר / הוצאת dist מגיט — נדחה במכוון
- [ ] 6 הסרת חוזי התאימות — נדחה במכוון
- [ ] בדיקה ידנית בטלפון אמיתי: משחק מלא אופליין עם שמע

### סבב האמינות הנוכחי

השינויים הבאים מוכנים בענפי תיקון; מצב המיזוג שלהם נקבע ב-PRs:

1. שמירה מדווחת על כשל ומשאירה את התוכן בעורך, עם אפשרות ייצוא וניסיון חוזר.
   סרגל העריכה נשאר נגיש מעל לוחות הסיבובים והשדות.
2. חוזה תוכן לפי משחק: createRound, validateRound וגרסה עם migrate אופציונלי.
   תוכן שמור נבדק בטעינה; מפתח localStorage הקיים נשמר. תוכן לא תקין אינו נמחק.
3. גרסאות אופליין נפרדות עם בדיקת שלמות לכל קובץ. התקנה שנכשלה אינה מחליפה
   גרסה עובדת. גרסה חדשה ממתינה לסגירת לשוניות ישנות; העורך ממשיך להיטען לפי דרישה.
4. בדיקות דפדפן הן שער לפריסה גם בדחיפה ישירה ל-main. נוסף כיסוי מגע ו-WebKit.

### חוזה הפיתוח לאחר המיגרציה

- runGame הוא עזר אופציונלי למשחקי סיבובים. המשחק מחזיק חוקים, תוכן ו-rendering.
- bootstrapGame מרכיב פתיחה, תוכן, שמע ועורך לפי צורך; כלי מורה אינם חייבים לולאת סיבובים.
- createRoundManager נשאר המימוש הפנימי של runGame וייצוא לתאימות.
- אחרי שינוי בקוד האתר או בנכסים מריצים npm run build ומחייבים את framework/dist.
  הבנייה מייצרת manifest עם מזהה גרסה שמבוסס על תוכן הקבצים ועל sw.js.
- אימות לפני מיזוג: `npm run verify` כולל בנייה נקייה וכל בדיקות הדפדפן.
  CI ופריסה חולקים action אחד; `verify:ci` בודק גם התאמה של dist ל-commit.
  בדיקות מובייל אוטומטיות אינן מחליפות בדיקת שמע בטלפון אמיתי.

---

## ארכיון: מצב `main` בעת כתיבת התכנית המקורית
...
**אם e2e נכשל — משווים לבסיס ובודקים אם הריפקטור שינה התנהגות.**
